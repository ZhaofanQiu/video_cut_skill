    ...
    def upload(self, file_path: str) -> str:
        ...
        try:
            ...
            if response.status_code != 200:
                raise FileUploadError(f"上传失败：状态码 {response.status_code}")
            ...
            raise FileUploadError(f"上传成功但无法获取文件ID。响应：{output}")
...
def upload_file_for_transcription(file_path: str, api_key: Optional[str] = None) -> str:
