...
class SmartTranscriber:
    ...
    MODEL_CONFIG = {
        ...
        ModelSize.BASE: {"speed": 7, "accuracy": 6, "memory_gb": 1, "best_for": "常规转录、短视频、输出片段", "available": True},  # 当前环境可用
        # 以下模型当前环境不支持，预留用于云端转录
        # ModelSize.SMALL: {
        #     'speed': 4,
        #     'accuracy': 7,
    ...
    def transcribe(self, video_path: str, model: ModelSize = None, language: str = "Chinese", is_output: bool = False) -> TranscriptResult:
        """智能转录.
    ...
    def refine_transcript(self, video_path: str, rough_transcript: TranscriptResult, use_cloud: bool = False) -> TranscriptResult:
    ...
    def _cloud_transcribe(self, video_path: str) -> TranscriptResult:
        ...
        """
        raise NotImplementedError("云端转录功能尚未实现。\n" "未来支持：\n" "  - OpenAI Whisper API\n" "  - 阿里云语音识别\n" "  - 腾讯云语音识别")
